    ...
    <div className="UserDetailPanel">
      <div className="BioSection">
        ...
        <div className="NameTag">
          ...
          <div className="UserRole">{userBio?.occupation || 'No Role Specified'}</div>
        </div>
      </div>
          ...
          chatSessions.map((session) => (
            <div key={session.id} className="SessionRow">
              <div className="SessionHeader">
                <span className="SessionTitle">{session.title || 'Chat Session'}</span>
                <span className="SessionDate">
                  {session.timestamp ? new Date(session.timestamp.seconds * 1000).toLocaleString() : ''}
                </span>
              </div>
              <div className="SessionReport">
                <strong>Initial Report:</strong> {session.initialReport || 'No report available.'}
              </div>
