...
const ProfileSettings = ({ onGoBack }) => {
  ...
  // Trigger hidden file input when "Edit Photo Profile" is clicked.
  ...
  const handlePasswordSubmit = async () => {
    try {
      // In production, reauthentication is required before updating the password.
  ...
  // Handle image upload via Cloudinary and update Firestore
  const handleImageUpload = async (e) => {
    const file = e.target.files[0];
    if (!file) return;
    const formData = new FormData();
    formData.append('file', file);
    // Use your unsigned preset name for profile images
    formData.append('upload_preset', 'unsigned_profile');
    try {
      // Use your Cloudinary cloud name (here 'dolqhixzw')
      ...
      if (data.secure_url) {
        ...
        // Update local state so that the new image is displayed immediately.
        setProfileImageUrl(data.secure_url);
        console.log("Profile image updated:", data.secure_url);
      } else {
